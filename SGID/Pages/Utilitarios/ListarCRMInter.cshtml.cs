...
{
    public class ListarCRMInterModel : PageModel
    {
        ...
        public ListarCRMInterModel(TOTVSINTERContext inter,ApplicationDbContext sgid)
        {
            ProtheusInter = inter;
            SGID = sgid;
        }
        public void OnGet()
        {
            Medicos = ProtheusInter.Sa1010s.Where(x => x.DELET != "*" && x.A1Clinter == "M" && x.A1Msblql != "1" && x.A1Vend!="" && x.A1Crm == "").OrderBy(x => x.A1Nvend).ToList();
            Total = Medicos.Count;
        }
