...
{
    public class AlterarProdutosInterModel : PageModel
    {
        ...
        public AlterarProdutosInterModel(TOTVSINTERContext denuo, ApplicationDbContext sgid, IWebHostEnvironment web)
        {
            Protheus = denuo;
            SGID = sgid;
            _WEB = web;
        ...
        public IActionResult OnPost(IFormCollection Anexos)
        {
                ...
                using (var reader = new StreamReader($"{Pasta}/TemporarioProdutoInter{Data}.csv"))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        if (line.Contains(';'))
                        {
                            ...
                            var ProdutoProtheus = Protheus.Sb1010s.FirstOrDefault(x => x.DELET != "*" && x.B1Cod == linha.B1Cod && x.B1Filial == "01");

                            produto = linha.B1Cod;

                            #region AlteracaoCampos
            ...
            catch (RankException e)
            {
                ...
                Logger.Log(e, SGID, "Alterar Produto Inter", user);
