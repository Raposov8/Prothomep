...
{
    [Authorize]
    public class CirurgiasFaturadasModel : PageModel
    {
        ...
        public CirurgiasFaturadasModel(TOTVSINTERContext context, ApplicationDbContext sgid)
        {
            Protheus = context;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostAsync(DateTime DataInicio, DateTime DataFim)
        {
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "CirurgiasFaturadasInter", user);
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                package.Workbook.Worksheets.Add("Cirurgias Faturadas AVANOS");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Cirurgias Faturadas AVANOS");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CirurgiasFaturadasAVANOS.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "CirurgiasFaturadasAVANOS Excel", user);
