...
{
    [Authorize]
    public class SubDistribuidorPorFabricanteModel : PageModel
    {
        ...
        public SubDistribuidorPorFabricanteModel(TOTVSDENUOContext protheus,ApplicationDbContext sgid)
        {
            Protheus = protheus;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostExport(string Fabricante,DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "SubDistribuidorPorFabricanteDenuo.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "SubDistribuidorPorFabricante Excel", user);
