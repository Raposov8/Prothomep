        ...
        public IActionResult OnPostExport(string NReduz)
        {
            try
            {
                ...
                lista.ForEach(Pedido =>
                {
                    ...
                    sheet.Cells[i, 2].Value = Pedido.Emissao;
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "SubDistribuidorNaoFaturado.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
