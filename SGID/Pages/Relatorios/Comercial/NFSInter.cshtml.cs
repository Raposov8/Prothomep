    ...
    public class NFSInterModel : PageModel
    {
        ...
        public IActionResult OnPostAsync(string Cliente, int Pagina = 1)
        {
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "NFSInter",user);
        ...
        public IActionResult OnPostExport()
        {
            try
            {
                ...
                package.Workbook.Worksheets.Add("NFS Inter");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "NFS Inter");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "NFSInter.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "NFSInter Excel", user);
