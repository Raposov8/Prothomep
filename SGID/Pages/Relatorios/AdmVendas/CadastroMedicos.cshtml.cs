...
{
    [Authorize]
    public class CadastroMedicosModel : PageModel
    {
        ...
        public CadastroMedicosModel(TOTVSDENUOContext context,ApplicationDbContext sgid)
        {
            Protheus = context;
            SGID = sgid;
        }
        public void OnGet(int id)
        {
            try
            {
                ...
                Relatorio = (from SA10 in Protheus.Sa1010s
                            ...
                            ).Skip((CurrentPage - 1) * 50).Take(50).ToList();
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "CadastroMedicos",user);
            }
        }
        ...
        public IActionResult OnPostExport()
        {
            try
            {
                var lista = (from SA10 in Protheus.Sa1010s
                             join SA30 in Protheus.Sa3010s on SA10.A1Vend equals SA30.A3Cod
                             where SA10.DELET != "*" && SA10.A1Msblql != "1" && SA10.A1Clinter == "M"
                             && SA30.DELET != "*"
                             orderby SA10.A1Cod descending
                ...
                package.Workbook.Worksheets.Add("Cadastro Médicos");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Cadastro Médicos");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CadastroMedicos.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "Cadastro Medicos Excel", user);
