        ...
        public IActionResult OnPostAsync(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                Relatorio = (from SC50 in Protheus.Sc5010s
                             ...
                             join PA10 in Protheus.Pa1010s on new { Filial = SC60.C6Filial, Patrim = SC60.C6Upatrim } equals new { Filial = PA10.Pa1Filial, Patrim = PA10.Pa1Codigo }
                             join SUA10 in Protheus.Sua010s on new { Filial = SC50.C5Filial, Proces = SC50.C5Uproces } equals new { Filial = SUA10.UaFilial, Proces = SUA10.UaNum }
                             where SC50.DELET != "*" && SC60.DELET != "*" && SA10.DELET != "*" && SB10.DELET != "*" && SA30.DELET != "*" && PA10.DELET != "*"
                             && SUA10.DELET != "*" && SC50.C5Liberok != "E" && (int)(object)SC50.C5XDtcir >= (int)(object)DataInicio.ToString("yyyy/MM/dd").Replace("/", "") && (int)(object)SC50.C5XDtcir <= (int)(object)DataFim.ToString("yyyy/MM/dd").Replace("/", "")
                             ...
                             select new RelatorioCirurgiasValorizadas
                             {
                                 ...
                                 ClienteFat = SC50.C5Cliente,
                                 LojaFat = SC50.C5Lojacli,
                                 ClienteFaturamento = SA10.A1Nome,
                                 Analista = SA10.A1Xatecli,
                                 Agendamento = SUA10.UaUnumage,
                                 ...
                                 Patrimonio = PA10.Pa1Despat ?? "SEM PATRIMONIO",
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "CirurgiasValorizadasInter",user);
        ...
        public IActionResult OnPostExport()
        {
            try
            {
                Relatorio = (from SC50 in Protheus.Sc5010s
                             ...
                             join SUA10 in Protheus.Sua010s on new { Filial = SC50.C5Filial, Proces = SC50.C5Uproces } equals new { Filial = SUA10.UaFilial, Proces = SUA10.UaNum }
                             where SC50.DELET != "*" && SC60.DELET != "*" && SA10.DELET != "*" && SB10.DELET != "*" && SA30.DELET != "*" && PA10.DELET != "*"
                             && SUA10.DELET != "*" && SC50.C5Liberok != "E" && (int)(object)SC50.C5XDtcir >= (int)(object)Inicio.ToString("yyyy/MM/dd").Replace("/", "") && (int)(object)SC50.C5XDtcir <= (int)(object)Fim.ToString("yyyy/MM/dd").Replace("/", "")
                             && SC50.C5Utpoper == "F" && SA10.A1Cgc != "04715053000140" && SA10.A1Cgc != "04715053000220" && SA10.A1Cgc != "01390500000140"
                             orderby SC50.C5XDtcir descending
                             select new RelatorioCirurgiasValorizadas
                             {
                ...
                sheet.Cells[1, 4].Value = "Cliente Fat";
                sheet.Cells[1, 5].Value = "Loja Fat";
                sheet.Cells[1, 6].Value = "Cliente Faturamento";
                sheet.Cells[1, 7].Value = "Analista";
                sheet.Cells[1, 8].Value = "Agendamento";
                sheet.Cells[1, 9].Value = "Tipo de Operação";
                sheet.Cells[1, 10].Value = "Vendedor";
                sheet.Cells[1, 11].Value = "Médico";
                sheet.Cells[1, 12].Value = "Paciente";
                sheet.Cells[1, 13].Value = "Cliente Entrega";
                sheet.Cells[1, 14].Value = "Convênio";
                sheet.Cells[1, 15].Value = "Num. Patrim.";
                sheet.Cells[1, 16].Value = "Patrimonio";
                sheet.Cells[1, 17].Value = "Produto";
                sheet.Cells[1, 18].Value = "Desc. Produto";
                sheet.Cells[1, 19].Value = "QTD Pedido";
                sheet.Cells[1, 20].Value = "QTD Faturada";
                sheet.Cells[1, 21].Value = "VL Unitário";
                sheet.Cells[1, 22].Value = "Total Pedido";
                sheet.Cells[1, 23].Value = "Total Fat.";
                sheet.Cells[1, 24].Value = "Faturado ?";
                ...
                Relatorio.ForEach(Pedido =>
                {
                    ...
                    sheet.Cells[i, 4].Value = Pedido.ClienteFat;
                    sheet.Cells[i, 5].Value = Pedido.LojaFat;
                    sheet.Cells[i, 6].Value = Pedido.ClienteFaturamento;
                    sheet.Cells[i, 7].Value = Pedido.Analista;
                    sheet.Cells[i, 8].Value = Pedido.Agendamento;
                    sheet.Cells[i, 9].Value = Pedido.TipoOper;
                    sheet.Cells[i, 10].Value = Pedido.Vendedor;
                    sheet.Cells[i, 11].Value = Pedido.Medico;
                    sheet.Cells[i, 12].Value = Pedido.Paciente;
                    sheet.Cells[i, 13].Value = Pedido.ClienteEnt;
                    sheet.Cells[i, 14].Value = Pedido.Convenio;
                    sheet.Cells[i, 15].Value = Pedido.NumPatrim;
                    sheet.Cells[i, 16].Value = Pedido.Patrimonio;
                    sheet.Cells[i, 17].Value = Pedido.Produto;
                    sheet.Cells[i, 18].Value = Pedido.DescProd;
                    sheet.Cells[i, 19].Value = Pedido.QTDPedido;
                    sheet.Cells[i, 20].Value = Pedido.QTDFaturada;
                    sheet.Cells[i, 21].Value = Pedido.VLUnitario;
                    sheet.Cells[i, 22].Value = Pedido.TotalPedido;
                    sheet.Cells[i, 23].Value = Pedido.TotalFat;
                    sheet.Cells[i, 24].Value = Pedido.Faturado;
                ...
                sheet.Cells[i, 19].Value = QtdPedido;
                sheet.Cells[i, 20].Value = QtdFaturada;
                sheet.Cells[i, 21].Value = VlUnitario;
                sheet.Cells[i, 22].Value = TotalPedido;
                sheet.Cells[i, 23].Value = TotalFat;

                sheet.Cells[i, 1, i, 18].Merge = true;
            ...
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "CirugiasValorizadasInter Excel",user);
