...
{
    [Authorize]
    public class ConsertoInterModel : PageModel
    {
        private ApplicationDbContext SGID { get; set; }

        public List<RelatorioConserto> Relatorio { get; set; } = new List<RelatorioConserto>();

        public ConsertoInterModel(ApplicationDbContext sgid)
        {
            SGID = sgid;
        }
        public void OnGet()
        {
            try
            {
                Relatorio = new List<RelatorioConserto>(); /*DB.NfdemoInters.Where(x => x.Tipo == "C")
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "ConsertoInter",user);
            }
        }
        ...
        public IActionResult OnPostExport()
        {
            try
            {
                ...
                package.Workbook.Worksheets.Add("Conserto Inter");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Conserto Inter");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "ConsertoInter.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "ConsertoInter Excel", user);
