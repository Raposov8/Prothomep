...
{
    [Authorize]
    public class NCMInterModel : PageModel
    {
        ...
        public NCMInterModel(TOTVSINTERContext context,ApplicationDbContext sgid)
        {
            Protheus = context;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostAsync(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                Relatorio = (from SFT in Protheus.Sft010s
                             ...
                             && (int)(object)SFT.FtEntrada >= (int)(object)DataInicio.ToString("yyyy/MM/dd").Replace("/", "") && (int)(object)SFT.FtEntrada <= (int)(object)DataFim.ToString("yyyy/MM/dd").Replace("/", "")
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "NCMInter",user);
        ...
        public IActionResult OnPostExport()
        {
            try
            {
                ...
                Relatorio = (from SFT in Protheus.Sft010s
                             ...
                             join SB10 in Protheus.Sb1010s on SFT.FtProduto equals SB10.B1Cod
                             where SFT.DELET != "*" && SFT2.DELET != "*" && SFT.FtTipomov == "E" && SFT2.FtTipomov == "S"
                             && (int)(object)SFT.FtEntrada >= (int)(object)Inicio.ToString("yyyy/MM/dd").Replace("/", "") && (int)(object)SFT.FtEntrada <= (int)(object)Fim.ToString("yyyy/MM/dd").Replace("/", "")
                             && SFT.FtNfori != "" && SFT.FtPosipi != SFT2.FtPosipi
                             select new RelatorioNCM
                             {
                                 Filial = SFT.FtFilial,
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "NCMInter.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "NCMInter Excel", user);
