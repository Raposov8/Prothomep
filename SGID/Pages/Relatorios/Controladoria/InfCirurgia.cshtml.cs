...
{
    [Authorize]
    public class InfCirurgiaModel : PageModel
    {
        ...
        public InfCirurgiaModel(TOTVSDENUOContext context, ApplicationDbContext sgid)
        {
            Protheus = context;
            SGID = sgid;
        }
        public string Nmage { get; set; }
        public string Flial { get; set; }

        public void OnGet()
        {
        }

        public IActionResult OnPostAsync(string Filial,string Numage)
        {
            try
            {
                Nmage = Numage;
                ...
                Relatorio = (from PAC10 in Protheus.Pac010s
                             ...
                             where PAC10.DELET != "*" && SA10.DELET != "*" && SA10.A1Msblql != "1" && SC50.DELET != "*" && PAC10.PacFilial == Filial && PAC10.PacNumage == Numage
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "InfCirurgias",user);
        ...
        public IActionResult OnPostExport()
        {
            try
            {
                ...
                Relatorio = (from PAC10 in Protheus.Pac010s
                             join SA10 in Protheus.Sa1010s on new { Cod = PAC10.PacClient, Loj = PAC10.PacLojent } equals new { Cod = SA10.A1Cod, Loj = SA10.A1Loja }
                             join SC50 in Protheus.Sc5010s on new { Filial = PAC10.PacFilial, Numpro = PAC10.PacNumpro } equals new { Filial = SC50.C5Filial, Numpro = SC50.C5Uproces }
                             where PAC10.DELET != "*" && SA10.DELET != "*" && SA10.A1Msblql != "1" && SC50.DELET != "*" && PAC10.PacFilial == Flial && PAC10.PacNumage == Nmage
                             && (SC50.C5Utpoper != "T" && SC50.C5Utpoper != "F")
                             select new RelatorioInfCirurgia
                             {
                                 PacNmPac = PAC10.PacNmpac,
                ...
                package.Workbook.Worksheets.Add("Inf Cirurgia");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Inf Cirurgia");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "InfCirurgia.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "InfCirurgia Excel", user);
