...
{
    [Authorize]
    public class DemonstracaoModel : PageModel
    {
        private ApplicationDbContext SGID { get; set; }

        public List<RelatorioConserto> Relatorio { get; set; } = new List<RelatorioConserto>();

        public DemonstracaoModel(ApplicationDbContext sgid)
        {
            SGID = sgid;
        }
        public void OnGet()
        {
            try
            {
                Relatorio = new List<RelatorioConserto>(); /*DB.NfdemoDenuos.Where(x => x.Tipo == "D")
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "Demonstracao",user);
            }
        }
        ...
        public IActionResult OnPostExport()
        {
            try
            {
                ...
                package.Workbook.Worksheets.Add("Demonstracao");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Demonstracao");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Demonstracao.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "Demonstracao Excel", user);
