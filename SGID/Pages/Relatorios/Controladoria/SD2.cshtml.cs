...
{
    [Authorize]
    public class SD2Model : PageModel
    {
        ...
        public SD2Model(TOTVSDENUOContext denuo,ApplicationDbContext sgid)
        {
            Protheus = denuo;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostExport(DateTime DataInicio,DateTime DataFim)
        {
            try
            {
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "SD2Denuo.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "SD2 Denuo Excel", user);
