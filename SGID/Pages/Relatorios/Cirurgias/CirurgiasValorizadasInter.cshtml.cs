    ...
    public class CirurgiasValorizadasInterModel : PageModel
    {
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                package.Workbook.Worksheets.Add("Cirurgias Valorizadas Inter");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Cirurgias Valorizadas Inter");
            ...
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "CirurgiasValorizadasInter Excel", user);
