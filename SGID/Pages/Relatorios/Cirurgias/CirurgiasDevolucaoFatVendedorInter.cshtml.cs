...
{
    [Authorize]
    public class CirurgiasDevolucaoFatVendedorInterModel : PageModel
    {
        ...
        public CirurgiasDevolucaoFatVendedorInterModel(TOTVSINTERContext context,ApplicationDbContext sgid)
        {
            Protheus = context;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostAsync(DateTime DataInicio, DateTime DataFim)
        {
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "CirurgiasDevolucaoFatVendedorInter",user);
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                package.Workbook.Worksheets.Add("Cirurgias Devolucao Fat Vendedor");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Cirurgias Devolucao Fat Vendedor");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CirurgiasDevolucaoFatVendedorInter.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "CirurgiasDevolucaoFatVendedorInter Excel", user);
