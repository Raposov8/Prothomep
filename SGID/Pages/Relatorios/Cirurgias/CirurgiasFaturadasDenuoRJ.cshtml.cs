...
{
    [Authorize]
    public class CirurgiasFaturadasDenuoRJModel : PageModel
    {
        ...
        public CirurgiasFaturadasDenuoRJModel(TOTVSDENUOContext context, ApplicationDbContext sgid)
        {
            Protheus = context;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostAsync(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                var query = (from SD20 in Protheus.Sd2010s
                             ...
                             select new
                             {
                                 ...
                                 SC50.C5Emissao,
                                 SA30.A3Nome,
                                 DataCirurgia = $"{SC50.C5XDtcir.Substring(6, 2)}/{SC50.C5XDtcir.Substring(4, 2)}/{SC50.C5XDtcir.Substring(0, 4)}",
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "CirurgiasFaturadasDenuoRJ",user);
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                package.Workbook.Worksheets.Add("Cirurgias Faturadas Denuo RJ");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Cirurgias Faturadas Denuo RJ");
                ...
                Relatorio.ForEach(Pedido =>
                {
                    ...
                    sheet.Cells[i, 4].Value = $"{Pedido.XDtcir.Substring(6, 2)}/{Pedido.XDtcir.Substring(4, 2)}/{Pedido.XDtcir.Substring(4)}";
                    sheet.Cells[i, 5].Value = Pedido.Nf;
                    sheet.Cells[i, 6].Value = Pedido.Serie;
                    sheet.Cells[i, 7].Value = $"{Pedido.Emissao.Substring(6, 2)}/{Pedido.Emissao.Substring(4, 2)}/{Pedido.Emissao.Substring(4)}";
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CirurgiasFaturadasDenuoRJ.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "CirurgiasFaturadasDenuoRJ Excel", user);
