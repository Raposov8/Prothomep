...
{
    [Authorize]
    public class CirurgiasValorizadasDenuoRJModel : PageModel
    {
        ...
        public CirurgiasValorizadasDenuoRJModel(TOTVSDENUOContext protheus,ApplicationDbContext sgid)
        {
            Protheus = protheus;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostAsync(DateTime DataInicio,DateTime DataFim)
        {
            try
            {
                ...
                Relatorio = (from SC50 in Protheus.Sc5010s
                             ...
                             select new RelatorioCirurgiasValorizadas
                             {
                                 ...
                                 Patrimonio = PA20.Pa1Despat ?? "SEM PATRIMONIO",
            ...
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "CirurgiasValorizadasDenuoRJ",user);
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                package.Workbook.Worksheets.Add("Cirurgias Valorizadas Denuo RJ");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "Cirurgias Valorizadas Denuo RJ");
                ...
                {
                    sheet.Cells[i, 1].Value = $"{Pedido.C5XDtCir.Substring(6, 2)}/{Pedido.C5XDtCir.Substring(4, 2)}/{Pedido.C5XDtCir.Substring(0, 4)}";
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CirurgiasValorizadasDenuoRJ.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "CirurgiasValorizadasDenuoRJ Excel", user);
