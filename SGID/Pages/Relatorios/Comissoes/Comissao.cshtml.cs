...
{
    [Authorize]
    public class ComissaoModel : PageModel
    {
        public TOTVSDENUOContext Protheus { get; set; }
        ...
        public ComissaoModel(TOTVSDENUOContext protheus, ApplicationDbContext sgid)
        {
            Protheus = protheus;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Comissao.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "Comissao Excel", user);
