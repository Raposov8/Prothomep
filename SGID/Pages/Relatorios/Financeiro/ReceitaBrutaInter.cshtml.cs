...
{
    public class ReceitaBrutaInterModel : PageModel
    {
        private TOTVSINTERContext Protheus { get; set; }
        private ApplicationDbContext SGID { get; set; }
        public List<ReceitaBruta> Relatorios { get; set; } = new List<ReceitaBruta>();
        public ReceitaBrutaInterModel(TOTVSINTERContext protheus, ApplicationDbContext sgid)
        {
            Protheus = protheus;
            SGID = sgid;
        }
        ...
        public IActionResult OnPost(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                Relatorios = (from SF20 in Protheus.Sf2010s
                              ...
                              && SF20.DELET != "*" && SD20.DELET != "*" && SB10.DELET != "*" && SD20.D2Quant != 0
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "Receita Bruta Inter", user);
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim, decimal Selic)
        {
            try
            {
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CheckMargemInter.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "Receita Bruta Inter Excel", user);
