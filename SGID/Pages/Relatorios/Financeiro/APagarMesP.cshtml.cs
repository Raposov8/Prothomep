...
{
    [Authorize]
    public class APagarMesPModel : PageModel
    {
        private TOTVSDENUOContext Protheus { get; set; }
        private ApplicationDbContext SGID { get; set; }
        public List<PagoMes> Relatorios { get; set; } = new List<PagoMes>();

        public APagarMesPModel(TOTVSDENUOContext protheus,ApplicationDbContext sgid)
        {
            Protheus = protheus;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostAsync(DateTime DataInicio, DateTime DataFim)
        {
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "APagarMesP",user);
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "APagarMesP.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "APagarMesP Excel", user);
