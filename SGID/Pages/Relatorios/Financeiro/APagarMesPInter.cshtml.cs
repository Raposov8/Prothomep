...
{
    [Authorize]
    public class APagarMesPInterModel : PageModel
    {
        ...
        public APagarMesPInterModel(TOTVSINTERContext protheus,ApplicationDbContext sgid)
        {
            Protheus = protheus;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostAsync(DateTime DataInicio,DateTime DataFim)
        {
            try
            {
                ...
                var query = (from SE20 in Protheus.Se2010s
                             ...
                             join SX50 in Protheus.Sx5010s on new { Tabela = "Z9", Chave = SED10.EdXgrdesp } equals new { Tabela = SX50.X5Tabela, Chave = SX50.X5Chave }
                             ...
                             select new
                             {
                                 ...
                                 VLPAGAR = SE20.E2Tipo == "PRE" ? 0 : SE20.E2Moeda == 1 ? SE20.E2Saldo + SE20.E2Sdacres - SE20.E2Sddecre : SE20.E2Moeda == 2 ? SE20.E2Valor * Moeda.Dolar : SE20.E2Moeda == 3 ? SE20.E2Valor * Moeda.Euro : 0,
                                 VLPRPAGAR = SE20.E2Tipo != "PRE" ? 0 : SE20.E2Moeda == 1 ? SE20.E2Saldo + SE20.E2Sdacres - SE20.E2Sddecre : SE20.E2Moeda == 2 ? SE20.E2Valor * Moeda.Dolar : SE20.E2Moeda == 3 ? SE20.E2Valor * Moeda.Euro : 0,
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "APagarMesPInter",user);
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                var query = (from SE20 in Protheus.Se2010s
                             join SED10 in Protheus.Sed010s on SE20.E2Naturez equals SED10.EdCodigo
                             join SX50 in Protheus.Sx5010s on new { Tabela = "Z9", Filial = "01", Chave = SED10.EdXgrdesp } equals new { Tabela = SX50.X5Tabela, Filial = SX50.X5Filial, Chave = SX50.X5Chave }
                             where SED10.DELET != "*" && SX50.DELET != "*" && SE20.DELET != "*" && SE20.E2Saldo != 0 && SE20.E2Tipo != "PA"
                             && SE20.E2Naturez.Substring(0, 3) != "111" && !(SE20.E2Tipo == "PR" && SE20.E2Prefixo == "EIC")
                             && (int)(object)SE20.E2Vencrea >= (int)(object)DataInicio.ToString("yyyy/MM/dd").Replace("/", "") && (int)(object)SE20.E2Vencrea <= (int)(object)DataFim.ToString("yyyy/MM/dd").Replace("/", "")
                             select new
                ...
                package.Workbook.Worksheets.Add("APagarMesP");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "APagarMesP");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "APagarMesPInter.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "APagarMesPInter Excel", user);
