...
namespace SGID.Pages.Relatorios.Estoque
{
    ...
    [Authorize]
    public class ExpedicaoRetiradaInterModel : PageModel
    {
        ...
        public ExpedicaoRetiradaInterModel(TOTVSINTERContext context,ApplicationDbContext sgid)
        {
            Protheus = context;
            SGID = sgid;
        }
        ...
        public IActionResult OnPostAsync(DateTime DataInicio,DateTime DataFim)
        {
            ...
            catch (Exception e)
            {
                ...
                Logger.Log(e, SGID, "ExpedicaoRetiradaInter",user);
        ...
        public IActionResult OnPostExport(DateTime DataInicio, DateTime DataFim)
        {
            try
            {
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "ExpedicaoRetiradaInter.xlsx");
            }
            catch (Exception e)
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(e, SGID, "ExpedicaoRetiradaInter Excel", user);
