...
namespace SGID.Pages.DashBoards
{
    ...
    public class DashBoardADMModel : PageModel
    {
        ...
        public double MetaInter { get; set; }
        public double MetaDenuo { get; set; }
        ...
        public IActionResult OnGet()
        {
            ...
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(ex, SGID, "DashBoardADM", user);
                return Page();
            }
        }

        public IActionResult OnPost(string MesInicio,string MesFim,string Ano)
        {
            try
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                string data = DateTime.Now.ToString("yyyy/MM").Replace("/", "");
                string DataInicio = Ano + MesInicio + "01";
                string DataFim = Ano + MesFim + "31";
                int[] CF = new int[] { 5551, 6551, 6107, 6109 };
