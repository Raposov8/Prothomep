...
{
    public class RelatorioComissaoDentalModel : PageModel
    {
        ...
        public TimeDentalRH Representante { get; set; }

        public List<RelatorioFaturamentoDental> Faturamento { get; set; } = new List<RelatorioFaturamentoDental>();

        public List<RelatorioDevolucaoDental> Devolucao { get; set; } = new List<RelatorioDevolucaoDental>();

        public RelatorioComissaoDentalModel(ApplicationDbContext sgid, TOTVSDENUOContext denuo)
        {
            SGID = sgid;
            ProtheusDenuo = denuo;
        }
        ...
        public void OnGet(string Mes, string Ano, int IdTime)
        {
            try
            {
                var Users = SGID.TimeDentals.FirstOrDefault(x=> x.Id == IdTime);
                ...
                var queryDevolucao = (from SD10 in ProtheusDenuo.Sd1010s
                ...
                var usuario = Users.Integrante.ToUpper();
               ...
               if (usuario != "MARCOS.PARRA")
               {
                   ...
                   this.Faturamento = Faturamento.Where(x => x.A3_LOGIN == usuario).ToList();

                    this.Devolucao = Devolucao.Where(x => x.Login == usuario).ToList();
               ...
               else
               {
                   ...
                   time.ComissaoEquipe = time.FaturadoEquipe * (time.User.PorcentagemSeg / 100);
                    ...
                    this.Faturamento = Faturamento.ToList();

                    this.Devolucao = Devolucao.ToList();
        ...
        public IActionResult OnPostExport(string Mes, string Ano, int IdTime)
        {
            try
            {
                ...
                var vendedores = Faturamento.Select(Pedido => new RelatorioVendedorDental
                {
                ...
                package.Workbook.Worksheets.Add("RelatorioComissaoRHDental");

                var sheet = package.Workbook.Worksheets.SingleOrDefault(x => x.Name == "RelatorioComissaoRHDental");
                ...
                return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"RelatorioComissoesRH{Mes}{Ano}{usuario.Replace(".", "")}.xlsx");
