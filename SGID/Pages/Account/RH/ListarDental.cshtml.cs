    ...
    public class ListarDentalModel : PageModel
    {
        ...
        public void OnGet()
        {
            try
            {
                ...
                Users = SGID.TimeDentals.Where(x => x.Status).ToList();
                ...
                var BaixaSubDenuo = (from SE50 in Protheus.Se5010s
                                     join SE10 in Protheus.Se1010s on new { Filial = SE50.E5Filial, PRE = SE50.E5Prefixo, Num = SE50.E5Numero, Par = SE50.E5Parcela, Tipo = SE50.E5Tipo, Cliente = SE50.E5Cliente, Loja = SE50.E5Loja }
                                     ...
                                     join SC50 in Protheus.Sc5010s on new { Filial = SE10.E1Filial, Pedido = SE10.E1Pedido } equals new { Filial = SC50.C5Filial, Pedido = SC50.C5Num } into sr
                                     from c in sr.DefaultIfEmpty()
                                     join SA30 in Protheus.Sa3010s on c.C5Vend1 equals SA30.A3Cod into Sa3
                                     from d in Sa3.DefaultIfEmpty()
                ...
                Users.ForEach(x =>
                {
                    ...
                    else
                    {
                        ...
                        time.FaturadoEquipe += BaixaSubDenuo.Where(x => x.Login == usuario).Sum(x => x.TotalBaixado);
        ...
        public IActionResult OnPost(string Mes, string Ano, string Empresa)
        {
            try
            {
                ...
                Users.ForEach(x =>
                {
                    ...
                    {
                        time.FaturadoEquipe += Faturamento.Sum(x => x.D2_TOTAL) + Devolucao.Sum(x => x.Total);
                        time.FaturadoEquipe += BaixaSubDenuo.Sum(x => x.TotalBaixado);
        ...
        public IActionResult OnPostExport(string Mes, string Ano, string Empresa)
        {
            try
            {
                ...
                Users.ForEach(x =>
                {
                    ...
                    {
                        time.FaturadoEquipe += Faturamento.Sum(x => x.D2_TOTAL) + Devolucao.Sum(x => x.Total);
                        time.Faturado += BaixaSubDenuo.Sum(x => x.TotalBaixado);
