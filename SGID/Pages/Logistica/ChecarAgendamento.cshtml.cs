...
{
    [Authorize]
    public class ChecarAgendamentoModel : PageModel
    {
        ...
        public ChecarAgendamentoModel(ApplicationDbContext sgid, IWebHostEnvironment wEB, TOTVSDENUOContext denuo, TOTVSINTERContext inter)
        {
            SGID = sgid;
            _WEB = wEB;
            ProtheusDenuo = denuo;
        ...
        public void OnGet(int id)
        {
            ...
            if (Agendamento.Empresa == "01")
            {
                ...
                SearchProduto = ProtheusInter.Sb1010s.Where(x => x.DELET != "*" && x.B1Msblql != "1" && x.B1Tipo != "KT" && x.B1Comerci=="C").Select(x => x.B1Desc).Distinct().ToList();
            }
            else
            {
                //Denuo
                ...
                SearchProduto = ProtheusDenuo.Sb1010s.Where(x => x.DELET != "*" && x.B1Msblql != "1" && x.B1Tipo != "KT" && x.B1Comerci=="C").Select(x => x.B1Desc).Distinct().ToList();
            }
        }
            ...
            List<Produto> Avulsos, List<Produto> Produtos, List<Patrimonio> Patris, string CodTabela, DateTime? Entrega,int Reprovado)
        {
            try
            {
                ...
                try
                {
                    ...
                    var mensagem = EmailTemplate.LerArquivoHtml($"{_WEB.WebRootPath}/template/TemplateRespostaEstoque.html", template);
            ...
            {
                string user = User.Identity.Name.Split("@")[0].ToUpper();
                Logger.Log(E, SGID, "ChecarAgendamento", user);
                return LocalRedirect($"/logistica/listarlogistica/{Empresa}/2");
            }
        }
    }
